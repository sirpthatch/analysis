...
class EtlModule(object):
    ...
    def create_composite_index(self, vals:list[str]) -> str:
        """
            Combines the values passed in to create a composite index
            Useful for creating partitions that are multiple values
        """
        return "__".join(vals)
    
    def split_composite_index(self, val:str) -> list[str]:
        """
            Decodes the supplied value back into a set of strings
        """
        return val.split("__")
    ...
    def verify_consistent(self, file_paths:list[Path]) -> None:
        for p in file_paths:
            ...
            if p.suffix.lower() != ".csv" and p.suffix.lower() != ".parquet":
                raise ValueError(f"{p} is not a CSV or Parquet file")
...
def ingest(src: Path, out: Path, name: str):
    ...
    available_paths = [p for p in src.glob(pattern="**/*") if p.is_file() and not p.name.startswith(".")]
